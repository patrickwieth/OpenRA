		...
		void QueueCaptureOrders(IBot bot)
		{
			...
			if (world.LocalRandom.Next(100) < Info.PriorityCaptureChance)
			{
				var priorityTargets = world.Actors.Where(a =>
					!a.IsDead && a.IsInWorld && Info.CapturablePlayerRelationships.HasStance(player.RelationshipWith(a.Owner))
			...
			var randPlayer = world.Players.Where(p => !p.Spectating
				&& Info.CapturablePlayerRelationships.HasStance(player.RelationshipWith(p))).Random(world.LocalRandom);
